...
「最近登录」面板逐条展示成功扫码登录的流水（时间、企微账号 userid、姓名、来源应用、登录发起 IP，最多保留 50 条并随统计文件持久化）。IP 为发起登录时浏览器来源地址（反代后取 `X-Real-IP`）；开启 `fetch_name` 后才有姓名。**行可点击**（悬浮呈手型光标），弹窗分「登录信息」与「成员信息」两组展示该用户的具体信息：企微账号、姓名、员工编码、部门（完整层级路径）；档案字段随 `fetch_profile` 采集，未开启或存量旧记录的相应字段显示为 `-`。该面板含用户账号信息，请妥善保管访问令牌，必要时在 Nginx 对 `/status`、`/api/status` 追加来源限制。
...
| `departments` | 所属部门列表，`name` 为完整层级路径（沿 `parentid` 向上追溯拼接，不含根部门/企业名）；应用可见范围外的部门名称为空串 |

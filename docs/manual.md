...
| `fetch_profile` | `false` | `true` 时额外获取成员信息：部门（完整层级路径，如「研发中心/研发部」）、员工编码，并含姓名，随 `/api/verify` 返回；需应用有通讯录读取权限。成员邮箱/企业邮箱受企微平台限制（2022-06-20 后新建自建应用不再返回），扫码链路不返回 |
| `job_number_extattr` | `员工编码` | 员工编码取自成员扩展属性的字段名（企微后台通讯录成员详情中的自定义字段，需企业自行维护）；扩展属性中找不到同名字段时 `job_number` 返回空串，不报错、不阻断登录 |

**`fetch_name` / `fetch_profile` 关系**：两者都是「取什么」的开关。组合行为对照：

| 配置组合 | 实际行为 |
| --- | --- |
| 只开 `fetch_name` | 只解析姓名（兼容旧配置，适合只要姓名的场景） |
| 开 `fetch_profile` | 档案全套解析（含姓名）：部门/员工编码用应用 token 即可获取；成员邮箱/企业邮箱受企微平台限制，扫码链路不返回 |
| 两者均不开启 | 不调通讯录接口，verify 仅返回 `userid` |

- 两者都开启时 `fetch_profile` 优先：姓名包含在档案同一次调用里，`fetch_name` 无需重复开启；
- 档案获取失败会自动降级并记录 Warn 日志，为空的字段不阻断登录；
- 成员邮箱/企业邮箱如确有需要，只能经企微内 H5 的 `snsapi_privateinfo` 手工授权获取（本服务暂未实现）。
...
「最近登录」面板逐条展示成功扫码登录的流水（时间、企微账号 userid、姓名、来源应用、登录发起 IP，最多保留 50 条并随统计文件持久化）。IP 为发起登录时浏览器来源地址（反代后取 `X-Real-IP`）；开启 `fetch_name` 后才有姓名。**行可点击**（悬浮呈手型光标），弹窗分「登录信息」与「成员信息」两组展示该用户的具体信息：企微账号、姓名、员工编码、部门（完整层级路径，值过长截断时鼠标悬浮可见完整内容）；信息字段随 `fetch_profile` 采集，未开启或存量旧记录的相应字段显示为 `-`。该面板含用户账号信息，请妥善保管访问令牌，必要时在 Nginx 对 `/status`、`/api/status` 追加来源限制。
...
| POST | `/api/verify` | 业务系统后端 | 签名/时间偏差校验 → ticket 一次性消费 → `{userid, name, job_number, departments, main_department}` |
...
# {"userid":"zhangsan","name":"张三","job_number":"10001","departments":[{"id":2,"name":"研发中心/研发部"}]}
...
| `departments` | 所属部门列表，`name` 为完整层级路径（沿 `parentid` 向上追溯拼接，不含根部门/企业名）；上级部门不在应用「可见范围」内时路径止于能查到的最上级，需完整路径请扩大应用可见范围 |

成员邮箱/企业邮箱受企微平台限制（2022-06-20 后新建自建应用），扫码链路不返回，响应中无此字段。
...
| 部门路径不完整（只有最内层） | 上级部门不在应用「可见范围」，祖先名称查不到（日志记 `部门信息查询失败 errcode=60003` Warn） | 企微后台扩大应用可见范围覆盖组织树；部门缓存 10 分钟后自动生效 |
...
开启 `status.enabled` 并配置 `status.token` 后，浏览器访问 `https://auth域名/status?token=你的令牌` 查看运行状态：近 7 天与今日的登录发起、ticket 签发、兑换成功统计，今日拒绝事件（state 失败 / 签名失败 / ticket 拒绝），最近登录流水（企微账号、姓名、来源应用与登录发起 IP，点击行可弹窗查看成员完整档案），版本与构建信息、运行时长、存储驱动与 Redis 在线状态，每 30 秒自动刷新。
